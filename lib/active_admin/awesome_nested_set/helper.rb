    ...
    module Helper
      ...
      def sortable_tree_columns
        column "&#9650;".html_safe do |resource|
          link_to("&#9650;".html_safe, [active_admin_config.route_instance_path(resource), :move_up].join("/"), :class => "arrow") if resource.left_sibling
        end
        column "&#9660;".html_safe do |resource|
          link_to("&#9660;".html_safe, [active_admin_config.route_instance_path(resource), :move_down].join("/"), :class => "arrow") if resource.right_sibling
        end
      end
